...
export class OnStakingPoolUnstake extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save OnStakingPoolUnstake entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save OnStakingPoolUnstake entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("OnStakingPoolUnstake", id.toString(), this);
    }
  }

  static load(id: string): OnStakingPoolUnstake | null {
    return changetype<OnStakingPoolUnstake | null>(
      store.get("OnStakingPoolUnstake", id)
    );
  }
...
export class DistributeRewards extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save DistributeRewards entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("DistributeRewards", id.toString(), this);
    }
  }

  static load(id: string): DistributeRewards | null {
    return changetype<DistributeRewards | null>(
      store.get("DistributeRewards", id)
    );
  }
